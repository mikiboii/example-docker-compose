...
client: TikTokLiveClient = TikTokLiveClient(unique_id="@stef_gamingg")
...
async def on_connect(event: ConnectEvent):
    ...
    # Store room info if available
    if client.room_info and 'owner' in client.room_info:
        tiktok_data['room_info'] = {
            'nickname': client.room_info['owner']['nickname'],
            'display_id': client.room_info['owner']['display_id'],
            'cover_url': client.room_info['cover']['url_list'][0] if client.room_info.get('cover') else None
        }
    tiktok_data['is_live'] = True
    tiktok_data['url'] = mm['data']['hd']['main']['flv']
...
async def check_once():
    try:
        ...
        tiktok_data['is_live'] = is_live
        
        if is_live:
            try:
                await client.connect(fetch_room_info=True)
            except Exception as e:
                tiktok_data['last_error'] = str(e)
                client.logger.error(f"Connection error: {e}")
        else:
            tiktok_data['last_error'] = "User is not currently live"
            
    except Exception as e:
        tiktok_data['last_error'] = str(e)
        client.logger.error(f"Error in check: {e}")
...
def miki():
        ...
        try:
            ...
            t = Thread(target=run_async_check)
            t.daemon = True
            t.start()
            
            while is_running:
                
                if tiktok_data['is_live']:

                    print("started streaming...")

                    try:
                            
                        app = Stream_demo(tiktok_data['url'], output_url)
